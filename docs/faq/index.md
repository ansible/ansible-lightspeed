...
The score returned from a k-nearest neighbor (kNN) search that is generated by a similarity function (which determines the distance between two vectors). The score is always greater than zero and larger scores imply a "shorter distance", or greater similarity, between two strings.
